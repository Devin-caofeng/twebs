...
int main(int argc, char **argv)
{
    ...
    if(dossl)
    {
        if((pid=Fork())==0)
        {
            ...
            while(1)
            {
                ...
                {
                    clienterror(connfd,
                                "maybe this web server not open to you!",
                                "403", "Forbidden",
    ...
    while (1)
    {
        ...
        if(access_ornot(inet_ntoa(clientaddr.sin_addr))==0)
        {
            clienterror(connfd,"maybe this web server not open to you!" ,
...
static void serve_dir(int fd,char *filename)
{
    ...
    while((dirp=readdir(dp))!=NULL)
    {
        ...
        sprintf(files,"%s<p><pre>%-2d %s ""<a href=%s%s"">%-15s\
                       </a>%-10s%10d %24s</pre></p>\r\n",
...
#ifdef HTTPS
    if(ishttps)  /* if ishttps, we could not redirct stdout to client, we must use SSL_write */
    {
        Pipe(httpsp);
